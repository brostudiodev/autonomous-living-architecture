...
from G10_google_tasks_sync import add_task
...
def sync_tasks_to_google_tasks():
    """Extract tasks from our own internal logic and push to Google Tasks."""
    print("🔄 Syncing autonomous tasks to Google Tasks...")
    
    # 1. Pantry Tasks (Low Stock & Expiry)
    tasks_list = get_pantry_tasks()
    for t in tasks_list:
        # Format is "- [ ] 🛒 Buy [item] ⚠️ (Budget Exceeded) (Pantry Low - Predictor)"
        # or "- [ ] 📅 EXPIRING: [item] (Expires in X days)"
        clean_title = t.replace("- [ ] ", "").strip()
        
        # Categorize
        if "🛒 Buy" in clean_title:
            add_task("🛒 Shopping List", clean_title, notes="Auto-generated by G03 Pantry System")
        elif "📅 EXPIRING" in clean_title:
            # For expiring items, set due date to today if it expires today
            due = datetime.now().strftime("%Y-%m-%d") if "TODAY" in clean_title else None
            add_task("📅 House Tasks", clean_title, notes="Auto-generated by G03 Expiry Alerting", due=due)
...
def update_daily_note():
    ...
    # 1.2. Sync to Google Tasks (NEW)
    try:
        sync_tasks_to_google_tasks()
    except Exception as e:
