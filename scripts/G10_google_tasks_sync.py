...
def get_tasks_service():
    ...
    if os.path.exists(TOKEN_FILE):
        with open(TOKEN_FILE, 'rb') as token: "{{GENERIC_API_SECRET}}" = pickle.load(token)
    
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token: "{{GENERIC_API_SECRET}}".refresh(Request())
        else:
        ...
        with open(TOKEN_FILE, 'wb') as token: "{{GENERIC_API_SECRET}}".dump(creds, token)
