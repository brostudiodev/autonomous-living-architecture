...
- **Verification Logic:** Creates a temporary `{db}_verify_test` database, restores the dump, and checks for public tables.
- **Encryption Logic:** Uses AES256 symmetric encryption via `gpg`.

# Failure Modes
| Scenario | Detection | Response |
|----------|-----------|----------|
| Verification Failed | Script logs PARTIAL/FAILURE | Alert via Telegram; check SQL file size |
