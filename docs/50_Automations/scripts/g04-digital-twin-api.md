...
## Endpoints
| Endpoint | Description | Response Field |
|---|---|---|
| `GET /status` | Full system summary (Health, Finance, Pantry). | `summary` / `response_text` |
| `GET /vision` | 🌟 2026 North Star and active Power Goal intents. | `response_text` |
| `GET /report` | 📊 Monthly strategic progress and roadmap audit. | `response_text` |
| `GET /tomorrow`| 🌙 Mission Briefing for next day (G10). | `response_text` |
| `GET /today` | 📱 Mobile dashboard from current daily note. | `response_text` |
| `GET /audit` | 🛡️ Self-healing integrity check (G11). | `response_text` |
| `GET /map` | 🗺️ G11 System connectivity matrix. | `response_text` |
| `GET /tasks` | Consolidated Google and Roadmap tasks. | `response_text` |
| `GET /health` | Raw biometric & recovery state. | raw JSON |
| `GET /finance` | Raw detailed budget metrics. | raw JSON |
...
```bash
{{ROOT_LOCATION}}/autonomous-living/.venv/bin/python {{ROOT_LOCATION}}/autonomous-living/scripts/G04_digital_twin_engine.py
```
