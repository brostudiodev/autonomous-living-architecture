...
# Logic Flow (Updated Apr 04)
1. **Directives:** Fetches the top-level mission from `morning_mission.txt`.
2. **Roadmaps:** Scans goal roadmaps for incomplete tasks **specifically for the current quarter** (e.g., Q2).
3. **Sentinels:** Pulls alerts from logistics, health, and relationship agents.
4. **Ranking:** Applies weighted scoring (Directive > Sentinel > Heal > Tasks > Wins).
