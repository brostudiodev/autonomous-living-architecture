...
4.  **Update `ACTIVITY_LOG.md`**: Appends a new entry to the goal's `ACTIVITY_LOG.md` file.
5.  **Commit `ACTIVITY_LOG.md`**: Commits the updated activity log.
...
## Manual Fallback
If the automation fails, the `EXECUTION.md` and `ACTIVITY_LOG.md` files can be updated manually in the GitHub repository.
