...
BEGIN
    ...
    -- 4) CRITICAL FIX: Handle Date Changes for Partitioned Table
    -- Delete any existing transaction with this ID that might have a DIFFERENT date
    DELETE FROM transactions WHERE transaction_id = _transaction_id AND transaction_date != _transaction_date;
