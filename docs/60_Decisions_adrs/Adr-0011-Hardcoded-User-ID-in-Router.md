...
A new `If` node (`Authorized1`) was added directly after the Telegram trigger. This node compares the incoming `user_id` from the Telegram message with a string from environment variables (`TELEGRAM_USER_ID`).
