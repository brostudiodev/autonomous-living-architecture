...
# S11: Meta-System Integration & Orchestration

## Executive Overview
The Meta-System Integration layer serves as the central nervous system and primary orchestrator of the autonomous infrastructure. It is responsible for multi-channel input routing (via n8n) and sequential, retry-aware synchronization of the entire ecosystem (via G11_global_sync).

**Core Value Proposition:** Ensures that the Digital Twin state is always consistent, fresh, and actionable by managing the "Global Heartbeat" and routing specialized intelligence between domains.

## Components
### 1. Global Sync Orchestrator (`G11_global_sync.py`)
The primary heartbeat script that ensures all data sources are synchronized.
- **Retry Logic:** Implements a 3-attempt cycle with 10-minute wait intervals to handle cloud data latency (e.g., Zepp Sync).
- **Registry:** Manages the execution sequence for 40+ domain scripts.
- **Observability:** Logs every execution to the `system_activity_log` for health monitoring.

### 2. ROUTER_Intelligence-Hub (n8n)
A sophisticated hub-and-spoke architecture that transforms chaotic multi-channel inputs (Telegram, Voice, PDF) into structured intelligence.
...
- **Telegram Authorization (Authorized1):** Validates against allowlisted user ID ({{TELEGRAM_CHAT_ID}})
...
## Stage 5: Strategic Intelligence & Routing

**Purpose:** Direct classified requests to appropriate specialized handlers while applying the system's "Strategic Brain."

**The Primary Directive (New):**
The router now integrates with the Digital Twin Engine's heuristic priority model. It identifies the single most critical task based on:
1. **Financial Integrity:** (e.g., budget breaches)
2. **Biological Sustainability:** (e.g., critical sleep debt)
3. **Life Logistics:** (e.g., document expiries)
4. **Physical Maintenance:** (e.g., overdue HIT sessions)
5. **Peak Performance:** (e.g., strategic deep work)

**Cross-Domain Correlation Engine:**
Identifies hidden patterns across systems, such as:
- Impact of low sleep on financial decision-making (spending anomalies).
- Correlation between environmental factors (temp/CO2) and sleep quality.
- Recovery-aware scheduling adjustments.
...
```
You are Michal's Intelligence Processing System.
